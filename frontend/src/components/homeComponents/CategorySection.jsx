...
const CategorySection = ({categoryButtons, handleNavigate}) => {
  return (
    <div className='flex flex-col w-full h-full gap-4 px-5 lg:p-20 md:px-10 lg:gap-10'>
        ...
        <div className='flex flex-row items-center w-full gap-1 lg:gap-3 text-secondary dark:text-darkSecondary'>
        ...
        <div className='grid items-center justify-between w-full h-full grid-cols-3 gap-3 p-3 border rounded-md lg:flex lg:flex-row border-text/10 dark:border-darkText/10 lg:overflow-x-scroll scrollbar_hidden lg:border-none'>
            ...
            {categoryButtons.map((categoryButton) => (
                <button onClick={() => handleNavigate(categoryButton.link)} key={categoryButton.id} className='flex flex-col items-center justify-center gap-1 p-2 rounded-md md:gap-2 md:py-5 md:flex-row min-w-fit bg-secondaryBackground dark:bg-darkSecondaryBackground lg:p-7 lg:flex-col lg:min-w-56'>
                    <i className='flex items-center text-mint lg:p-4 lg:rounded-full lg:bg-text/5 dark:lg:bg-darkText/5'>{categoryButton.icon}</i>
